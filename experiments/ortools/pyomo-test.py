...
def active_constraint_rule(model, vm, t):
    ...
    return model.vm_active[vm, t] <= (1 if t >= 0 else 0) * (model.start[vm] <= t) * (
        t <= model.start[vm] + migration_duration - 1
    )
...
model.vm_active_start_con = pyo.Constraint(
    model.VMS, model.TIME, rule=active_start_rule
)
...
def active_end_rule(model, vm, t):
    ...
    M = planning_horizon
    return t <= model.start[vm] + migration_duration - 1 + M * (
        1 - model.vm_active[vm, t]
    )


model.vm_active_end_con = pyo.Constraint(model.VMS, model.TIME, rule=active_end_rule)
...
def cumulative_constraint_rule(model, t):
    ...
    return (
        sum(migration_throughput[vm] * model.vm_active[vm, t] for vm in model.VMS)
        <= host_maximum_throughput
    )


model.cumulative_con = pyo.Constraint(model.TIME, rule=cumulative_constraint_rule)
...
model.makespan_con = pyo.Constraint(model.VMS, rule=makespan_constraint_rule)
