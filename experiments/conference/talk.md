...
My name is Atalay Kutlay, I'm a senior software engineer at Akamai's Network Optimization team. My main focus is solving optimization problems to increase Akamai's fleet efficiency, I've worked on ranging from CDN capacity planning to virtual machine allocation, (pause) and most recently large scale maintenance scheduling, which I will be talking about today.

I want to start my talk with a scenario. Consider yourself as the SRE team that manages thousands of hypervisor hosts that serve virtual machines that belong to external customers. Your job is to keep this fleet healthy and up-to-date. Now, on Monday morning your boss tells you that there is a new kernel that has an important fix and there are 10,000 machines that need to be updated. For this update, there is no live patching available, so the host needs to be rebooted, which means, all of the virtual machines need to be evacuated before you can do the maintenance. You have to notify the customers in advance, since the migration of their VMs will mean some downtime on their end. You have 90 days.
...
All these questions need to be answered daily while picking the hosts that will be updated. I hope you see that this is not a trivial problem. In fact, it's a quite complex scheduling problem which our SRE teams tackled manually with lots of effort and coordination. 
...
As a cloud provider, it's our responsibility, and a huge concern for our SRE teams, to keep our fleet healthy and up-to-date. That entails being on top of maintenance which includes kernel and OS updates, firmware updates, and also the preventative host hardare maintenance. These are the planned maintenance activities that we do and most of them require the hosts to be rebooted and VMs to be evacuated. 

Now you may ask, why do we have do migrate? Why can't we reboot the VMs with the hosts? First reason not to do that is the risk of host not booting back after the reboot due to various potential issues. The second is, we want to keep the customer's notification as flexible as possible. If we were to schedule a reboot of the whole host, all customers would need to be rebooted at the same time, making it difficiult to schedule.

-- 3:30 minutes

There are many ways to migrate a VM, which would be 

Fleet maintenance is difficult primarily because of scale. We need to evacuate and update a large number of hosts in a short time so that software updates finish quickly. 

This forces us to decide which hosts to evacuate at any given time, and that decision must carefully account for 3 things:
